...
const Register = () => {
  ...
  const {signIn, googleLogin, setUser,user}= useContext(AuthContext)
  ...
  const handleRegister = (e) => {
    ...
    signIn(email, passowrd).then(res=>{
      const user = res.user;
      updateProfile(user, {
        displayName: name,
        photoURL: PhotoUrl
    ...
    <div className=" mt-10 min-h-screen max-w-full mx-auto px-4">
      <div className="bg-base-100 w-full max-w-xl mx-auto shadow-2xl flex flex-col p-6 sm:p-10 items-center rounded-lg">
        ...
        <div className="w-full">
          ...
          <button className="btn bg-white text-black border-[#e5e5e5] w-full " onClick={handleGoogle}>
            <svg
